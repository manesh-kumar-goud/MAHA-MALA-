-- =====================================================
-- MAHALAXMI SOLAR ENERGIES - COMPLETE DATABASE SCHEMA
-- =====================================================
...
-- =====================================================
-- CONTACT INQUIRIES TABLE (Updated - email is optional)
-- =====================================================
CREATE TABLE contact_inquiries (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name VARCHAR(255) NOT NULL,
  email VARCHAR(255), -- Made optional to match code
  phone VARCHAR(20),
  subject VARCHAR(500),
...
-- Public access policies (for contact forms and public content)
CREATE POLICY "Anyone can insert contact inquiries" ON contact_inquiries FOR INSERT WITH CHECK (TRUE);
CREATE POLICY "Admins can view all contact inquiries" ON contact_inquiries FOR SELECT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
CREATE POLICY "Admins can update contact inquiries" ON contact_inquiries FOR UPDATE USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
...
-- Leads policies
CREATE POLICY "Users can view their own leads" ON leads FOR SELECT USING (referrer_id = auth.uid());
CREATE POLICY "Users can insert their own leads" ON leads FOR INSERT WITH CHECK (referrer_id = auth.uid());
CREATE POLICY "Admins can view all leads" ON leads FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);

-- Bank details policies
CREATE POLICY "Users can manage their own bank details" ON bank_details FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Admins can view all bank details" ON bank_details FOR SELECT USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
...
CREATE POLICY "Admins can manage subsidy info" ON subsidy_info FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
...
-- 2. Create your first admin user manually (or through your app)
-- 3. Update the user's role in the users table to 'admin' or 'super_admin'
-- 4. Update your .env.local file with the new Supabase credentials
